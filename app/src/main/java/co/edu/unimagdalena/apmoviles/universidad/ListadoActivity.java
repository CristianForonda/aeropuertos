...
public class ListadoActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    SearchView filter;
    AeropuertoCursorAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor c = aeropuertoController.allAeropuertos2(); //en esta linea es donde se traen los datos de la base de datos
        adapter = new AeropuertoCursorAdapter(this,c,false);//se pasan los datos por el adaptador
        listado.setAdapter(adapter);//se ponen los datos en la lista
        listado.setTextFilterEnabled(true);
        adapter.setFilterQueryProvider(new FilterQueryProvider() {
            @Override
            public Cursor runQuery(CharSequence constraint) {
                return aeropuertoController.filtrarAeropuerto(constraint);
            }
        });
